...
OUTPUT_DIR = ROOT / "outputs" / "full_market_lgbm_broadened_candidates"
...
def main() -> None:
    ...
    broad_candidates = panel[_broad_candidate_mask(panel)].copy()
    strict_candidates["year"] = strict_candidates["date"].dt.year
    broad_candidates["year"] = broad_candidates["date"].dt.year
    ...
    for year in [2023, 2024, 2025, 2026]:
        ...
        test = broad_candidates[broad_candidates["year"] == year].copy()
    ...
    pred.to_csv(OUTPUT_DIR / "broad_candidate_predictions.csv", index=False)
    ...
    runs = [
        ...
        ("broad_lgbm_rank_2023_oos", "broad_lgbm"),
        ("broad_hybrid_rank_2023_oos", "broad_hybrid"),
        ("broad_lgbm_top2_each_day_2023_oos", "broad_lgbm_top2"),
    ]
    summaries = []
...
def _strict_candidate_mask(df: pd.DataFrame, config: dict) -> pd.Series:
    ...
    return _shared_candidate_mask(df, entry, universe_mask, rs20_pool, rs60_pool, rs20_cutoff, rs60_cutoff)


def _broad_candidate_mask(df: pd.DataFrame) -> pd.Series:
    universe_mask = df["fundamental_rank"] <= 30
    pool_counts = universe_mask.groupby(df["date"]).transform("sum").clip(lower=1)
    rs20_pool = df["ret20"].where(universe_mask).groupby(df["date"]).rank(ascending=False, pct=True)
    rs60_pool = df["ret60"].where(universe_mask).groupby(df["date"]).rank(ascending=False, pct=True)
    rs20_cutoff = np.maximum(0.70, 1 / pool_counts)
    rs60_cutoff = np.maximum(0.80, 1 / pool_counts)
    return (
        universe_mask
        ...
        & (df["ret20"] > df["benchmark_ret20"] * 0.5)
        & (df["ret60"] > df["benchmark_ret60"] * 0.5)
        & (df["ret5"] <= 0.15)
        & (df["ret20"] <= 0.65)
...
def _shared_candidate_mask(
    df: pd.DataFrame,
    entry: dict,
    universe_mask: pd.Series,
    rs20_pool: pd.Series,
    rs60_pool: pd.Series,
    rs20_cutoff: pd.Series | np.ndarray,
    rs60_cutoff: pd.Series | np.ndarray,
) -> pd.Series:
    return (
        universe_mask
...
def _run_backtest(
    ...
    for idx, date in enumerate(dates[:-1]):
        ...
        if free_slots > 0:
            ...
            mask = _strict_candidate_mask(day_frame, config) if mode == "strict_base" else _broad_candidate_mask(day_frame)
