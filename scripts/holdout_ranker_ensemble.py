...
OUTPUT_DIR = ROOT / "outputs" / "holdout_ranker_ensemble"
...
def main() -> None:
    ...
    scored = holdout_candidates.copy()
    for name, model in models.items():
        scored[name] = model.predict(_clean_features(scored[FEATURE_COLUMNS]))
    for col in models:
        scored[f"{col}_pct"] = scored.groupby("date")[col].rank(pct=True)
    ...
    scored["stable_ensemble_score"] = (
        scored.groupby("symbol")["ensemble_score"].transform(lambda s: s.rolling(5, min_periods=1).mean())
    )
    predictions = scored[
        [
            "date",
    ...
    runs = [
        ("reg_forward60_stop15", "reg_forward60", 0.15),
        ("ranker_stop15", "ranker_score", 0.15),
        ("ensemble_stop15", "ensemble_score", 0.15),
        ("stable_ensemble_stop15", "stable_ensemble_score", 0.15),
        ("ensemble_stop12", "ensemble_score", 0.12),
    ]
    summaries = []
    for name, score_col, stop_loss in runs:
        equity, trades = _run_backtest(scored_panel, data, config, pd.Timestamp("2025-01-01"), score_col, stop_loss)
...
def _fit_regressor(train: pd.DataFrame, target: str, seed: int) -> lgb.LGBMRegressor:
...
def _fit_classifier(train: pd.DataFrame, target: str, seed: int) -> lgb.LGBMClassifier:
    ...
    model.fit(_clean_features(train[FEATURE_COLUMNS]), train[target].astype(int))
    return _ClassifierProbWrapper(model)


def _fit_ranker(train: pd.DataFrame, target: str, seed: int) -> lgb.LGBMRanker:
    train = train.sort_values(["date", "symbol"]).copy()
    group = train.groupby("date").size().to_numpy()
    ...
    model.fit(_clean_features(train[FEATURE_COLUMNS]), train[target].astype(int), group=group)
    return model


class _ClassifierProbWrapper:
...
def _add_rank_labels(panel: pd.DataFrame) -> pd.DataFrame:
    df = panel.copy()
    pct = df.groupby("date")["forward_60d_return"].rank(pct=True)
    df["fwd60_rank_label"] = np.floor(pct.fillna(0) * 4).clip(0, 3).astype(int)
    top = df.groupby("date")["forward_60d_return"].rank(ascending=False, pct=True) <= 0.2
    df["top_quantile_60"] = top.astype(float)
    df.loc[df["forward_60d_return"].isna(), ["fwd60_rank_label", "top_quantile_60"]] = np.nan
    return df
...
def _run_backtest(
    ...
    for idx, date in enumerate(dates[:-1]):
        ...
        if free_slots > 0:
            ...
            candidates = day_frame[_candidate_mask(day_frame)].copy()
            if not candidates.empty and score_col in candidates:
                ...
                candidates = candidates.sort_values([score_col, "final_score"], ascending=False)
...
def _metrics(scored: pd.DataFrame) -> pd.DataFrame:
    rows = []
    score_cols = ["reg_forward60", "reg_reward60", "cls_topq60", "rank_forward60", "ensemble_score", "stable_ensemble_score"]
    for year, group in scored.groupby(scored["date"].dt.year):
            ...
            rows.append(
                {
                    ...
                    "rank_corr": _rank_corr(group["forward_60d_return"].to_numpy(), group[score_col].to_numpy()),
                    "top_half_forward60": group.loc[group[score_col] >= group[score_col].median(), "forward_60d_return"].mean(),
                    "bottom_half_forward60": group.loc[group[score_col] < group[score_col].median(), "forward_60d_return"].mean(),
                }
            )
